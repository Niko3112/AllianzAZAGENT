...
document.addEventListener('DOMContentLoaded', () => {
  ...
  hamburger.addEventListener('click', () => {
    // Prvo čitamo trenutno stanje
    const isOpen = navLinks.classList.contains('open');

    // Zatim radimo sve promjene (pisanje)
    navLinks.classList.toggle('open');
    if (!isOpen) { // Meni se otvara
      hamburgerSpans[0].style.transform = 'translateY(7px) rotate(45deg)';
      hamburgerSpans[1].style.opacity = '0';
      hamburgerSpans[2].style.transform = 'translateY(-7px) rotate(-45deg)';
    } else {
  ...
  if (lightbox) {
    ...
    const openLightbox = (sourceImg) => {
      ...
      if (navLinks.classList.contains('open')) hamburger.click();
